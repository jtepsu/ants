...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lower_bound=0, upper_bound=float('inf')):
        ...
        distance = 0
        while nearest_place.is_hive == False and distance <= upper_bound:
            if len(nearest_place.bees) > 0 and distance >= lower_bound:
        ...
        while not isinstance(place, AntHomeBase):
            if place.ant is not None:
                if place.ant.is_container and place.ant.ant_contained is not None:
                    if hasattr(place.ant.ant_contained, 'damage'):
                        place.ant.ant_contained.double()
                if hasattr(place.ant, 'damage'):
                    place.ant.double()
            place = place.exit
